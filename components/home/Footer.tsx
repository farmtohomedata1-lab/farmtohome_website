...
import { IconChat, IconPhone, IconPin, IconSend, socialIcons } from "./icons";
...
export default function Footer() {
  return (
    ...
    >
      <div className="mx-auto grid w-full max-w-[1320px] gap-10 px-4 py-14 sm:px-6 md:grid-cols-2 lg:grid-cols-[1.3fr_1fr_1fr_1.3fr]">
        <div>
          <Logo prefix={footer.logoPrefix} suffix={footer.logoSuffix} />
        ...
        <div>
          ...
          <ul className="mt-5 space-y-5 text-sm">
            ...
            <li className="flex items-start gap-3">
              ...
              <span>
                <span className="block text-gray-400">{footer.help.hours}</span>
                <a
            ...
            <li className="flex items-start gap-3">
              <IconChat className="mt-0.5 h-5 w-5 shrink-0 text-brand-green" />
              <span>
                <span className="block font-semibold">
                  {footer.help.chatTitle}
                </span>
                <span className="mt-1 block text-gray-400">
                  {footer.help.chatSubtitle}
                </span>
              </span>
            </li>
          </ul>
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex w-full max-w-[1320px] flex-col items-center justify-between gap-4 px-4 py-5 sm:flex-row sm:px-6">
          <p className="text-xs text-gray-400">{footer.copyright}</p>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400">{footer.paymentLabel}</span>
            <ul className="flex items-center gap-2">
              {footer.payments.map((payment) => (
                <li
                  key={payment}
                  className="rounded bg-white px-2 py-1 text-[10px] font-bold text-gray-800"
                >
                  {payment}
                </li>
              ))}
